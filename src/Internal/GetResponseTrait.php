    ...
    protected function parseXmlByType($searchPath, $key, &$value, $xml, $prefix)
    {
            ...
            if ($type === 'array') {
                ...
                if ($type === 'object') {
                    ...
                    foreach ($properties as $pkey => $pvalue) {
                        ...
                        $array[$pkey] = $this->parseXmlByType($searchPath.'/'.$prefix.$name, $name, $pvalue, $xml,
                            $prefix);
                    }
                    return $array;
                }
        ...
        if ($result = $xml->xpath($searchPath)) {
            ...
            if ($type === 'boolean') {
                ...
                if ($type === 'numeric' || $type === 'float' || $type === 'int' || $type === 'integer') {
                    return null;
                } else {
                    return '';
                }
                                    ...
                                    if (isset($value['type']) && $value['type'] === 'json') {
                                        ...
                                        if ($jsonBody && ($data = json_decode($jsonBody, true))) {
                                            if (is_array($data)) {
                                                ...
                                                ObsLog::commonLog(ERROR, "response body %s, and jsonToArray data is %s",
                                                    $body, $data);
                                            }
                                        }
                                        $closeBody = true;
