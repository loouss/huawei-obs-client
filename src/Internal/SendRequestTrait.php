...
trait SendRequestTrait
{
    ...
    public function createV4SignedUrl(array $args = []): Model
    {
        ...
        $timestamp = $date ? date_create_from_format('D, d M Y H:i:s \G\M\T', $date,
        ...
        $canonicalstring = $v4->makeCanonicalstring($method, $headers, $_queryParams, $bucketName, $objectKey,
    ...
    public function createV4PostSignature(array $args = []): Model
    {
        ...
        foreach ($formParams as $key => $val) {
            if ($key) {
                ...
                if (!in_array($key, Constants::ALLOWED_REQUEST_HTTP_HEADER_METADATA_NAMES) && strpos($key,
                        V2Constants::HEADER_PREFIX) !== 0 && !in_array($key, $conditionAllowKeys)) {
    ...
    public function __call($originMethod, $args)
    {
        ...
        if (!$async) {
            ...
            ObsLog::commonLog(INFO,
                'obsclient cost '.round(microtime(true) - $start, 3) * 1000 .' ms to execute '.$originMethod);
            unset($model['method']);
            return $model;
        } else {
    ...
    protected function makeRequest($model, &$operation, $params, $endpoint = null)
    {
        ...
        $signatureInterface = strcasecmp($this->signature, 'v4') === 0 ?
            new V4Signature($this->ak, $this->sk, $this->pathStyle, $endpoint, $this->region, $model['method'],
                $this->signature, $this->securityToken, $this->isCname) :
            new DefaultSignature($this->ak, $this->sk, $this->pathStyle, $endpoint, $model['method'], $this->signature,
                $this->securityToken, $this->isCname);
    ...
    protected function sendRequestAsync(
        ...
        return $this->httpClient->sendAsync($request, ['stream' => $saveAsStream])->then(
            function (Response $response) use (
                ...
                if ($statusCode === 307 && !$readable) {
                    if ($location = $response->getHeaderLine('location')) {
                        ...
                        $scheme = (isset($newUrl['scheme']) ? $newUrl['scheme'] : $url['scheme']);
                        $defaultPort = strtolower($scheme) === 'https' ? '443' : '80';
                        return $this->doRequestAsync($model, $operation, $params, $callback, $startAsync, $originMethod,
                            $scheme.'://'.$newUrl['host'].
                            ':'.(isset($newUrl['port']) ? $newUrl['port'] : $defaultPort));
                    }
                }
                $this->parseResponse($model, $request, $response, $operation);
                ObsLog::commonLog(INFO,
                    'obsclient cost '.round(microtime(true) - $startAsync, 3) * 1000 .' ms to execute '.$originMethod);
                unset($model['method']);
                $callback(null, $model);
            },
            function (RequestException $exception) use (
                ...
                if ($exception instanceof ConnectException) {
                    if ($requestCount <= $this->maxRetryCount) {
                        return $this->sendRequestAsync($model, $operation, $params, $callback, $startAsync,
                            $originMethod, $request, $requestCount + 1);
                    } else {
                        $message = 'Exceeded retry limitation, max retry count:'.$this->maxRetryCount.', error message:'.$exception->getMessage();
                    }
                }
